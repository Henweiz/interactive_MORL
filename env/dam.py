...
class Dam(gym.Env, EzPickle):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if self.initial_state is not None:
            ...
            if not self.penalize:
                state = self.np_random.choice(Dam.s_init, size=1)
            else:
    ...
    def _render_gui(self, render_mode: str):
        ...
        # Draw the wall
        for y in range(0, int(Dam.H_FLO_U), self.wall_img.get_width()):
        ...
        self.window.blit(img, (20, self.window_size[1] - Dam.H_FLO_U))
        pygame.draw.line(
            self.window,
            (255, 0, 0),
            (0, self.window_size[1] - Dam.H_FLO_U),
            (self.window_size[0], self.window_size[1] - Dam.H_FLO_U),
        )
    ...
    def step(self, action):
        ...
        self.dam_inflow = self.np_random.normal(Dam.DAM_INFLOW_MEAN, Dam.DAM_INFLOW_STD, len(self.state))[0]
        # small chance dam_inflow < 0
        n_state = np.clip(self.state + self.dam_inflow - action, 0, None).astype(np.float32)

        # cost due to excess level wrt a flooding threshold (upstream)
        self.excess = np.clip(n_state / Dam.S - Dam.H_FLO_U, 0, None)[0]
        r0 = -self.excess + penalty
        # deficit in the water supply wrt the water demand
        self.defict = -np.clip(Dam.W_IRR - action, 0, None)[0]
        r2 = self.defict + penalty


        q = np.clip(action[0] - Dam.Q_MEF, 0, None)
        p_hyd = Dam.ETA * Dam.G * Dam.GAMMA_H2O * n_state[0] / Dam.S * q / 3.6e6

        # deficit in hydroelectric supply wrt hydroelectric demand
        r1 = -np.clip(Dam.W_HYD - p_hyd, 0, None) + penalty
        # cost due to excess level wrt a flooding threshold (downstream)
        r3 = -np.clip(action[0] - Dam.Q_FLO_D, 0, None) + penalty
